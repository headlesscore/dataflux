...
namespace ThoughtWorks.CruiseControl.UnitTests
{
	public class CustomAssertion
	{
		public static void AssertContains(string search, string target)
		{
			...
			ClassicAssert.IsTrue(target.IndexOf(search) >= 0, message);
		...
		public static void AssertNotContains(string search, string target)
		{
			...
			ClassicAssert.IsFalse(target.IndexOf(search) >= 0, message);
		}
		...
		public static void AssertStartsWith(string expected, string actual)
		{
			...
			ClassicAssert.IsTrue(actual != null && actual.StartsWith(expected), message);
		}
		...
		public static void AssertMatches(string pattern, string actual)
		{
			...
			ClassicAssert.IsTrue(Regex.IsMatch(actual, pattern), message);
		}
		...
		public static void AssertMatchCount(int expectedCount, string pattern, string actual)
		{
			...
			ClassicAssert.AreEqual(expectedCount, actualCount, message);
		}

		public static void AssertFalse(bool assert)
		{
			ClassicAssert.IsTrue(!assert);
		}

		public static void AssertFalse(string message, bool assert)
		{
			ClassicAssert.IsTrue(!assert, message);
		}
		...
		public static void AssertEquals(Type expectedType, object actual)
		{
			...
			{
				ClassicAssert.AreEqual(expectedType, actual);
				return;
			}
			ClassicAssert.IsNotNull(actual, string.Format(System.Globalization.CultureInfo.CurrentCulture,"object of expected type {0} is null", expectedType.FullName));
			Type actualType = (actual is Type) ? (Type) actual : actual.GetType();
			ClassicAssert.AreEqual(expectedType, actualType, "object of the wrong type");
		}
		...
		public static void AssertNotEquals(object expected, object actual)
		{
			...
			ClassicAssert.IsTrue(!expected.Equals(actual), message);
			ClassicAssert.IsTrue(!actual.Equals(expected), message);
		}
		...
		public static void AssertApproximatelyEqual(string message, double expected, double actual, double tolerance)
		{
			...
			ClassicAssert.IsTrue(Math.Abs(expected - actual) < tolerance, message + expectation);
		}

		public static void AssertEqualArrays(Array expected, Array actual)
		{
			ClassicAssert.AreEqual(expected.Length, actual.Length, "Arrays should have same length");

			for (int i = 0; i < expected.Length; i++)
			{
				ClassicAssert.AreEqual(expected.GetValue(i), actual.GetValue(i), "Comparing array index " + i);
			}
		}

		public static void AssertXPathExists(string xml, string xpath)
		{
			ClassicAssert.IsTrue(SelectNodeIterator(xpath, xml).Count > 0,
