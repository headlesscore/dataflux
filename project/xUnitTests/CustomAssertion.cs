...
namespace ThoughtWorks.CruiseControl.UnitTests
{
    public class CustomAssertion
    {
        public static void AssertContains(string search, string target)
        {
            ...
            Assert.True(target.IndexOf(search) >= 0, message);
        }
        ...
        public static void AssertNotContains(string search, string target)
        {
            ...
            Assert.False(target.IndexOf(search) >= 0, message);
        }
        ...
        public static void AssertStartsWith(string expected, string actual)
        {
            ...
            Assert.True(actual != null && actual.StartsWith(expected), message);
        }
        ...
        public static void AssertMatches(string pattern, string actual)
        {
            ...
            Assert.True(Regex.IsMatch(actual, pattern), message);
        }
        ...
        public static void AssertEquals(Type expectedType, object actual)
        {
            ...
            {
                Assert.True(expectedType == (Type) actual);
                return;
            }
            Assert.False(actual is null, string.Format(System.Globalization.CultureInfo.CurrentCulture, "object of expected type {0} is null", expectedType.FullName));
            Type actualType = (actual is Type) ? (Type)actual : actual.GetType();
            Assert.True(expectedType == actualType, "object of the wrong type");
        }
        ...
        public static void AssertApproximatelyEqual(string message, double expected, double actual, double tolerance)
        {
            ...
            Assert.True(Math.Abs(expected - actual) < tolerance, message + expectation);
        }

        public static void AssertEqualArrays(Array expected, Array actual)
        {
            Assert.True(expected.Length == actual.Length, "Arrays should have same length");

            for (int i = 0; i < expected.Length; i++)
            {
                Assert.True(expected.GetValue(i) == actual.GetValue(i), "Comparing array index " + i);
            }
        }

        public static void AssertXPathExists(string xml, string xpath)
        {
            Assert.True(SelectNodeIterator(xpath, xml).Count > 0,
        ...
        public static void AssertXPathNodeValue(string expectedValue, string xml, string xpath)
        {
            ...
            Assert.True(expectedValue == actual,
        ...
        private static XPathNodeIterator SelectNodeIterator(string xpath, string xml)
        {
            ...
            {
                throw new Exception(string.Format(System.Globalization.CultureInfo.CurrentCulture, "Unable to parse xml <{0}> or xpath expression <{1}>\n\t{2}", xml, xpath, e));
            }
        }
    }
}
