...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Queues
{
	...
	public class IntegrationQueueTest
	{
        ...
        {
            Assert.NotNull(integrationQueueUseFirst.Configuration);
        }
		...
		public void SecondIntegrationRequestForQueuedProjectShouldNotStartImmediately()
		{
			...
			IIntegrationQueueItem[] itemsOnQueue = integrationQueueUseFirst.GetQueuedIntegrations();
			Assert.Equal(2, itemsOnQueue.Length);
			Assert.Same(integrationQueueItem1, itemsOnQueue[0]);
			Assert.Same(secondQueueItem, itemsOnQueue[1]);
		...
		public void TwoProjectsWithSameQueueNameShouldShareQueue()
		{
			...
			Assert.Equal(2, itemsOnQueue.Length);
			Assert.Same(integrationQueueItem1, itemsOnQueue[0]);
			Assert.Same(integrationQueueItem2, itemsOnQueue[1]);
		...
		public void DequeueForSingleQueuedItemClearsQueue()
		{
			...
			IIntegrationQueueItem[] itemsOnQueue = integrationQueueUseFirst.GetQueuedIntegrations();
			Assert.Equal(0, itemsOnQueue.Length);
		...
		public void DequeueForMultipleQueuedItemsActivatesNextItemOnQueue()
		{
			...
			IIntegrationQueueItem[] itemsOnQueue = integrationQueueUseFirst.GetQueuedIntegrations();
			Assert.Equal(1, itemsOnQueue.Length);
			Assert.Same(integrationQueueItem2, itemsOnQueue[0]);
		...
		public void ProjectsWithSamePriorityShouldBeInEntryOrder()
		{
			...
			IIntegrationQueueItem[] itemsOnQueue = integrationQueueUseFirst.GetQueuedIntegrations();
			Assert.Equal(3, itemsOnQueue.Length);
			Assert.Same(integrationQueueItem1, itemsOnQueue[0]);
			Assert.Same(integrationQueueItem2, itemsOnQueue[1]);
			Assert.Same(integrationQueueItem3, itemsOnQueue[2]);
		...
		public void ProjectsWithNonZeroPriorityInFrontOfZeroPriority()
		{
			...
			Assert.Equal(3, itemsOnQueue.Length);
			Assert.Same(integrationQueueItem1, itemsOnQueue[0]);
			Assert.Same(integrationQueueItem3, itemsOnQueue[1]);
			Assert.Same(integrationQueueItem2, itemsOnQueue[2]);
		...
		public void GetNextRequestSucceedsWhenFirstQueueItemIsThisProject()
		{
			...
			IntegrationRequest ir = integrationQueueUseFirst.GetNextRequest((IProject)project1Mock.Object);
			Assert.Same(ir, integrationRequestForceBuild);
			VerifyAll();
		}
        ...
        public void ProveThatQueueLocksBehaveCorrectlyWhenAcquiredAndReleased()
        {
            ...
            // starting position
            Assert.False(integrationQueues[queues[0]].IsBlocked, "Initial state of Queue0 should be unblocked");
            Assert.False(integrationQueues[queues[1]].IsBlocked, "Initial state of Queue1 should be unblocked");
            Assert.False(integrationQueues[queues[2]].IsBlocked, "Initial state of Queue2 should be unblocked");

            // Toggle locks on #0 (acquire)
            IDisposable lock1, lock2;
            Assert.True(integrationQueues[queues[0]].TryLock(out lock1));
            Assert.False(integrationQueues[queues[0]].IsBlocked, "State of Queue0 should be unlocked");
            Assert.True(integrationQueues[queues[1]].IsBlocked, "State of Queue1 should be locked");
            Assert.True(integrationQueues[queues[2]].IsBlocked, "State of Queue2 should be locked");

            // Toggle locks on #0 (release)
            lock1.Dispose();
            Assert.False(integrationQueues[queues[0]].IsBlocked, "State of Queue0 should be unlocked");
            Assert.False(integrationQueues[queues[1]].IsBlocked, "State of Queue1 should be unlocked");
            Assert.False(integrationQueues[queues[2]].IsBlocked, "State of Queue2 should be unlocked");

            // Toggle locks on #1 (acquire)
            Assert.True(integrationQueues[queues[1]].TryLock(out lock1));
            Assert.True(integrationQueues[queues[0]].IsBlocked, "State of Queue0 should be locked");
            Assert.False(integrationQueues[queues[1]].IsBlocked, "State of Queue1 should be unlocked");
            Assert.False(integrationQueues[queues[2]].IsBlocked, "State of Queue2 should be unlocked");

            // Toggle locks on #2 (acquire) - this means 2 queues have a lock on #0, therefore both must be released
            Assert.True(integrationQueues[queues[2]].TryLock(out lock2));
            Assert.True(integrationQueues[queues[0]].IsBlocked, "State of Queue0 should be locked");
            Assert.False(integrationQueues[queues[1]].IsBlocked, "State of Queue1 should be unlocked");
            Assert.False(integrationQueues[queues[2]].IsBlocked, "State of Queue2 should be unlocked");
        ...
        public void ProjectsInQueueShouldNotIntegrateIfQueueIsLocked()
        {
            ...
            Assert.Null(integrationQueueReplace.GetNextRequest((IProject)project1Mock.Object));
