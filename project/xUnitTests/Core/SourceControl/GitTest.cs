...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Sourcecontrol
{
	...
	public class GitTest : ProcessExecutorTestFixtureBase
	{
		...
		{
			Assert.Equal("git", git.Executable, "#A1");
            Assert.True(true);
        }
...
<git>
	...
	<committerEMail>max.mustermann@gmx.de</committerEMail>
			...
			Assert.Equal("git", git.Executable, "#B1");
			Assert.Equal(@"c:\git\ccnet\mygitrepo", git.Repository, "#B2");
			Assert.Equal("master", git.Branch, "#B3");
			Assert.Equal(new Timeout(5), git.Timeout, "#B4");
			Assert.Equal(@"c:\git\working", git.WorkingDirectory, "#B5");
			Assert.Equal(true, git.TagOnSuccess, "#B6");
			Assert.Equal(true, git.AutoGetSource, "#B7");
			Assert.Equal("CCNet Test Build {0}", git.TagCommitMessage, "#B8");
			Assert.Equal("{0}", git.TagNameFormat, "#B9");
			Assert.Equal("Max Mustermann", git.CommitterName, "#B10");
			Assert.Equal("max.mustermann@gmx.de", git.CommitterEMail, "#B11");
			Assert.Equal(true, git.CommitBuildModifications, "#B12");
			Assert.Equal(true, git.CommitUntrackedFiles, "#B13");
            Assert.Equal(500, git.MaxAmountOfModificationsToFetch,  "#B14");
...
<git>
    <repository>c:\git\ccnet\mygitrepo</repository>
			...
			git = (Git)NetReflector.Read(xml);
			Assert.Equal(@"git", git.Executable, "#C1");
			Assert.Equal(@"c:\git\ccnet\mygitrepo", git.Repository, "#C2");
			Assert.Equal(@"master", git.Branch, "#C3");
			Assert.Equal(new Timeout(600000), git.Timeout, "#C4");
			Assert.Equal(null, git.WorkingDirectory, "#C5");
			Assert.Equal(false, git.TagOnSuccess, "#C6");
			Assert.Equal(true, git.AutoGetSource, "#C7");
			Assert.Equal("CCNet Build {0}", git.TagCommitMessage, "#C8");
			Assert.Equal("CCNet-Build-{0}", git.TagNameFormat, "#C9");
			Assert.Equal(null, git.CommitterName, "#C10");
			Assert.Equal(null, git.CommitterEMail, "#C11");
			Assert.Equal(false, git.CommitBuildModifications, "#C12");
			Assert.Equal(false, git.CommitUntrackedFiles, "#C13");
            Assert.Equal(100, git.MaxAmountOfModificationsToFetch, "#C14");
		}
