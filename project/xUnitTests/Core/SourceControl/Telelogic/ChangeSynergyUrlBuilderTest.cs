...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Sourcecontrol.Telelogic
{
	...
	public class ChangeSynergyUrlBuilderTest
	{
		...
		public virtual void SetUp()
		{
			...
			Assert.NotNull(synergy.UrlBuilder);
			Assert.True(synergy.UrlBuilder is ChangeSynergyUrlBuilder);
			synergyUrlBuilder = synergy.UrlBuilder as ChangeSynergyUrlBuilder;
			Assert.NotNull(synergyUrlBuilder);
            Assert.True(true);
            Assert.True(true);
        }
		...
		{
			Assert.Same(synergy.Connection.Database, synergyUrlBuilder.Database);
			Assert.Equal(@"http://myserver:8060", synergyUrlBuilder.Url);
			Assert.Equal(@"User", synergyUrlBuilder.Role);
			Assert.Equal("csuser", synergyUrlBuilder.Username);
			Assert.False("readonly" == synergyUrlBuilder.Password);
		}
		...
		public void PasswordObfuscation()
		{
			...
			Assert.Equal("0:0,0,0,0,0,0,0,0", synergyUrlBuilder.ObfuscatePassword(0, "password"));
			Assert.Equal("1:112,97,115,115,119,111,114,100", synergyUrlBuilder.ObfuscatePassword(1, "password"));
			Assert.Equal("999:111888,96903,114885,114885,118881,110889,113886,99900", synergyUrlBuilder.ObfuscatePassword(999, "password"));
			Assert.Equal("1000:112000,97000,115000,115000,119000,111000,114000,100000", synergyUrlBuilder.ObfuscatePassword(1000, "password"));

			Assert.Equal("500:45500,42000,52000,34500,16000,40500,58500,16500,33500,53500,16000,33000,57000,24000,59500,55000,16000,35000,55500,47000,16000,53000,42500,54500,56000,18000,16000,39500,59000,25500,57000,16000,58000,52000,50500,16000,62000,32000,61000,60500,16000,50000,55500,51500,62500", synergyUrlBuilder.ObfuscatePassword(500, "[ThE Qu!Ck Br0wn Fo^ jUmp$ Ov3r the |@zy dog}"));
		}
		...
		public virtual void SetupModification()
		{
			...
			for (int i = 0; i < size; i++)
			{
				...
				Assert.Equal(expectedUrls[i], testModifications[i].Url);
			}
		}
	...
	public class ChangeSynergyLoginTest : ChangeSynergyUrlBuilderTest
	{
		...
		public override void Config()
		{
			...
			Assert.Equal(@"http://myserver:8060", synergyUrlBuilder.Url);
			Assert.Equal(@"User", synergyUrlBuilder.Role);
			Assert.Null(synergyUrlBuilder.Username);
			Assert.Null(synergyUrlBuilder.Password);
		}
