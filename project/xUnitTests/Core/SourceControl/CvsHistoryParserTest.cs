...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Sourcecontrol
{
	...
	public class CvsHistoryParserTest : CustomAssertion
	{
		...
		public void ParseStream() 
		{
			...
			Assert.Equal(6, modifications.Length);
            Assert.True(true);
			...
			Assert.Equal(mod5, modifications[0]);
			Assert.Equal(mod4, modifications[1]);
			Assert.Equal(mod1, modifications[2]);
			Assert.Equal(mod2, modifications[3]);
			Assert.Equal(mod3, modifications[4]);
			Assert.Equal(mod6, modifications[5]);
		}
		...
		public void VerifyModifiedFile() 
		{
			...
			Assert.Equal("modified", mods[0].Type);
		}
...
=============================================================================";
			...
			Assert.Equal(2, mods.Length);
			Assert.Equal("1.5", mods[0].Version);
			Assert.Equal("owen", mods[0].UserName);
			Assert.Equal("Run.cs", mods[0].FileName);
			Assert.Equal("modified", mods[0].Type);
			Assert.Equal(@"C:\dev\CVSRoot/fitwebservice/src/fitwebservice/src", mods[0].FolderName);
		}
...
=============================================================================";
			...
			Assert.Equal("owen", mods[0].UserName);
			Assert.Equal("modified", mods[0].Type);
			Assert.Equal(@"C:\dev\CVSRoot/fitwebservice/src/fit webservice/src", mods[0].FolderName);
			Assert.Equal("Run File.cs", mods[0].FileName);
		}
