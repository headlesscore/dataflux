...
";
            ...
            Assert.NotNull(mods);
            Assert.True(true);
            Assert.Equal(expectedModifications.Length, mods.Length);
            for (int i = 0; i < expectedModifications.Length; i++)
            {
                Assert.Equal(expectedModifications[i].ChangeNumber, mods[i].ChangeNumber);
                Assert.Equal(expectedModifications[i].Comment, mods[i].Comment);
                Assert.Equal(expectedModifications[i].EmailAddress, mods[i].EmailAddress);
                Assert.Equal(expectedModifications[i].FileName, mods[i].FileName);
                Assert.Equal(expectedModifications[i].FolderName, mods[i].FolderName);
                Assert.Equal(expectedModifications[i].ModifiedTime, mods[i].ModifiedTime);
                Assert.Equal(expectedModifications[i].Type, mods[i].Type);
                Assert.Equal(expectedModifications[i].Url, mods[i].Url);
                Assert.Equal(expectedModifications[i].UserName, mods[i].UserName);
                Assert.Equal(expectedModifications[i].Version, mods[i].Version);
            }
        }
