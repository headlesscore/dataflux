...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Sourcecontrol.Bitkeeper
{
	...
	public class BitKeeperHistoryParserTest
	{
		...
		public void ParseChangeSets()
		{
            ...
            Assert.Equal("user@host.(none)", mods[0].UserName);
			Assert.Equal("  Remove file in subdir.", mods[0].Comment);
			Assert.Equal("ChangeSet", mods[0].Type);
			Assert.Equal("ChangeSet", mods[0].FileName);
			Assert.Equal("", mods[0].FolderName);
		}
		...
		public void ParseRenamed()
		{
			...
			Assert.Equal("user@host.(none)", mods[1].UserName);
			Assert.Equal("    Rename: asubdir/baz.txt -> asubdir2/bazzz.txt", mods[1].Comment);
			Assert.Equal("Renamed", mods[1].Type);
			Assert.Equal("bazzz.txt", mods[1].FileName);
			Assert.Equal("asubdir2", mods[1].FolderName);
		}
		...
		public void ParseDeletions()
		{
			...
			Assert.Equal("user@host.(none)", mods[1].UserName);
			Assert.Equal("    Delete: asubdir/baz.txt", mods[1].Comment);
			Assert.Equal("Deleted", mods[1].Type);
			Assert.Equal("baz.txt", mods[1].FileName);
			Assert.Equal("asubdir", mods[1].FolderName);
		}
		...
		public void ParseAdditions()
		{
			...
			Assert.Equal("user@host.(none)", mods[1].UserName);
			Assert.Equal("    BitKeeper file /var/lib/bk/demo/dev-1.0/asubdir/baz.txt", mods[1].Comment);
			Assert.Equal("Added", mods[1].Type);
			Assert.Equal("baz.txt", mods[1].FileName);
			Assert.Equal("asubdir", mods[1].FolderName);
		}
		...
		public void ParseVerboseModifications()
		{
			...
			Assert.Equal(21, mods.Length);
			foreach (Modification mod in mods)
			{
				Assert.Equal("user@host.(none)", mod.UserName);
				Assert.NotNull("filename should not be null");
			}
		}
		...
		public void ParseNonVerboseModifications()
		{
			...
			foreach (Modification mod in mods)
			{
				Assert.Equal("user@host.(none)", mod.UserName);
				Assert.NotNull("filename should not be null");
				Assert.Equal("ChangeSet", mod.Type);
			}
		}
