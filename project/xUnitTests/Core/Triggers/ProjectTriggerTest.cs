namespace ThoughtWorks.CruiseControl.UnitTests.Core.Triggers
{
	...
	<innerTrigger type=""intervalTrigger"">
		...
		<seconds>10</seconds>
            ...
            Assert.Equal("http://fooserver:12342/CruiseManager.rem", trigger.ServerUri);
            Assert.Equal("Foo", trigger.Project);
            Assert.NotNull(trigger.InnerTrigger);
            Assert.Equal(IntegrationStatus.Failure, trigger.TriggerStatus);
            Assert.True(trigger.TriggerFirstTime);
        }
        ...
        public void PopulateFromMinimalConfiguration()
        {
            ...
            var trigger = NetReflector.Read(xml) as ProjectTrigger;
            Assert.NotNull(trigger);
            Assert.Equal("tcp://localhost:21234/CruiseManager.rem", trigger.ServerUri);
            Assert.Equal("Foo", trigger.Project);
            Assert.NotNull(trigger.InnerTrigger);
            Assert.Equal(IntegrationStatus.Success, trigger.TriggerStatus);
            Assert.False(trigger.TriggerFirstTime);
        }
