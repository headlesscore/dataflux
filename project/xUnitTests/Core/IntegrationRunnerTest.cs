...
namespace ThoughtWorks.CruiseControl.UnitTests.Core
{
    ...
    public class IntegrationRunnerTest : IntegrationFixture
    {
        ...
        public void ShouldNotRunBuildIfResultShouldNotBuild()
        {
            ...
            Assert.True(Directory.Exists(result.ArtifactDirectory));
            mockery.Verify();
        }
        ...
        public void GenerateSystemParameterValuesShouldAddNewParameters()
        {
            ...
            Assert.Equal(2, request.BuildValues.Count);
            Assert.True(request.BuildValues.ContainsKey("$CCNetUser"));
            Assert.Equal("John Doe", request.BuildValues["$CCNetUser"]);
            Assert.True(request.BuildValues.ContainsKey("$CCNetLabel"));
            Assert.Equal("1.1", request.BuildValues["$CCNetLabel"]);
            Assert.Equal(2, parameters.Count);
            Assert.Equal(2, parameters.Count);
            Assert.Equal("$CCNetUser", parameters[0].Name);
            Assert.Equal("John Doe", parameters[0].Value);
            Assert.Equal("$CCNetLabel", parameters[1].Name);
            Assert.Equal("1.1", parameters[1].Value);
        }
        ...
        public void GenerateSystemParameterValuesShouldUpdateExistingParameters()
        {
            ...
            Assert.Equal("John Doe", request.BuildValues["$CCNetUser"]);
            Assert.True(request.BuildValues.ContainsKey("$CCNetLabel"));
            Assert.Equal("1.2", request.BuildValues["$CCNetLabel"]);
            Assert.Equal(2, parameters.Count);
            Assert.Equal("$CCNetUser", parameters[0].Name);
            Assert.Equal("John Doe", parameters[0].Value);
            Assert.Equal("$CCNetLabel", parameters[1].Name);
            Assert.Equal("1.2", parameters[1].Value);
        }
        #endregion
    }
}
