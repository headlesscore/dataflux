...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Tasks
{
    ...
    public class SynchronisationTaskTests
    {
        ...
        public void RunFailsIfTaskErrors()
        {
            ...
            Assert.Equal(IntegrationStatus.Failure, result.Status);
            Assert.NotNull(result.ExceptionResult);
            Assert.Equal("Task failed!", result.ExceptionResult.Message);
        }
        ...
        public void RunAllowsTasksInSeparateContexts()
        {
            ...
            Assert.True(ThreadPool.QueueUserWorkItem(t1 =>
            {
                task1.Run(result1);
                event1.Set();
            }));
            Assert.True(ThreadPool.QueueUserWorkItem(t2 =>
            {
                task2.Run(result2);
                event2.Set();
            }));
            ...
            Assert.Equal(IntegrationStatus.Success, result1.Status);
            Assert.Equal(IntegrationStatus.Success, result2.Status);
        }
        ...
        public void RunTimesOutIfContextNotAvailable()
        {
            ...
            {
                Assert.Equal(IntegrationStatus.Failure, result1.Status);
            }
            else
            {
                Assert.Equal(IntegrationStatus.Failure, result2.Status);
            }
        }
