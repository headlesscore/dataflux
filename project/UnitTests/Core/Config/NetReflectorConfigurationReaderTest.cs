...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Config
{
	...
	public class NetReflectorConfigurationReaderTest
	{
        ...
        public void DeserialiseSecurityFromXml()
        {
            ...
            IConfiguration configuration = reader.Read(ConfigurationFixture.GenerateConfig(projectXml + queueXml), null);
            ClassicAssert.That(configuration.SecurityManager, Is.InstanceOf<NullSecurityManager>());
            ClassicAssert.That(configuration.SecurityManager, Is.InstanceOf<NullSecurityManager>());
            ClassicAssert.That(configuration.SecurityManager, Is.InstanceOf<NullSecurityManager>());
        }
        ...
        public void DeserialiseSingleProjectPlusUnknownFromXml()
        {
            ...
            ClassicAssert.That(delegate { reader.Read(ConfigurationFixture.GenerateConfig(projectXml + queueXml), null); },
		...
		public void DeserialiseCustomProjectFromXml()
		{
			...
			ClassicAssert.IsNotNull(configuration.Projects["foo"]);
			ClassicAssert.IsTrue(configuration.Projects["foo"] is CustomTestProject);
			ClassicAssert.AreEqual("foo", ((CustomTestProject) configuration.Projects["foo"]).Name);
		}
		...
		public void DeserialiseProjectFromXmlWithUnusedNodesShouldGenerateEvent()
		{
			...
			ClassicAssert.That(delegate {  reader.Read(ConfigurationFixture.GenerateConfig(xml), null); },
                        Throws.TypeOf<ConfigurationException>());
		}
		...
		public void AttemptToDeserialiseProjectWithMissingXmlForRequiredProperties()
		{
			...
			ClassicAssert.That(delegate { reader.Read(ConfigurationFixture.GenerateConfig(projectXml), null); },
                        Throws.TypeOf<ConfigurationException>());
		}
		...
		{
			ClassicAssert.That(delegate { reader.Read(new XmlDocument(), null); },
                        Throws.TypeOf<ConfigurationException>());
		}
		...
		{
			ClassicAssert.That(delegate { reader.Read(XmlUtil.CreateDocument("<loader/>"), null); },
                        Throws.TypeOf<ConfigurationException>());
		}
		...
		private void ValidateProject(IConfiguration configuration, string projectName)
		{
			...
			ClassicAssert.AreEqual(projectName, project.Name);
			ClassicAssert.IsTrue(project.Tasks[0] is NullTask);
			ClassicAssert.IsTrue(project.SourceControl is NullSourceControl);
			ClassicAssert.AreEqual(1, project.Publishers.Length);
			ClassicAssert.IsTrue(project.Publishers[0] is NullTask);
		}
